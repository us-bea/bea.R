...
bea2Tab <- function(beaPayload, asWide = TRUE, iTableStyle = TRUE) {
	...
	if('error' %in% tolower(
	  ...
	  warning('API response error. Please print your response payload for details.')
...
#		)
	...
	if(is.null(attributes(beaResults)$params)){
		warning('Request parameter data not found; returned values may not contain successful BEA API response.')
	}
